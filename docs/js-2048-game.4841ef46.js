...
document.addEventListener('keydown', (e)=>{
    ...
    if (game.status === 'win') messageWin.classList.remove('hidden');
...
const board = ()=>{
    ...
    for(let i = 0; i < BOARD_SIZE; i++)for(let j = 0; j < BOARD_SIZE; j++){
        const cells = [
            ...rows[i].querySelectorAll('.field-cell')
        ];
   ...
   */ constructor(initialState = [
    ...
    moveLeft() {
        ...
        for(let i = 3; i >= 0; i--){
            ...
            for(let j = newRow.length; j < this.size; j++)newRow.push(0);
            newState[i] = newRow;
        }
        for(let i = 0; i <= this.size - 1; i++){
            for(let n = 0; n <= this.size - 1; n++)if (newState[i][n] !== this.state[i][n]) canMove = true;
        }
        if (canMove) {
            this.state = newState;
            this.addCell();
    ...
    moveRight() {
        ...
        for(let i = 3; i >= 0; i--){
            ...
            for(let j = newRow.length; j < this.size; j++)newRow.unshift(0);
            newState[i] = newRow;
        }
        for(let i = 0; i <= 3; i++){
    ...
    updateState(state) {
        const newState = [];
        for(let i = 0; i <= this.size - 1; i++)newState.push([
            state[0][i],
            state[1][i],
            state[2][i],
            state[3][i]
    ...
    isWin() {
        for(let i = 0; i <= this.size - 1; i++){
            for(let n = 0; n <= this.size - 1; n++)if (this.state[i][n] === this.lastGameScore) this.status = 'win';
        }
    }
    isLose() {
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++){
            if (this.state[i][j] === 0) return;
            if (i < 3 && this.state[i][j] === this.state[i + 1][j] || j < 3 && this.state[i][j] === this.state[i][j + 1]) return;
        }
        this.status = 'lose';
    }
}
